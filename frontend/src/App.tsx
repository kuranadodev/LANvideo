...
export default function App() {
  const [settings, setSettings] = useState<AppSettings | null>(null); const [saved, setSaved] = useState<AppSettings | null>(null); const [status, setStatus] = useState<any>(null); const [videoDevices, setVideoDevices] = useState<Device[]>([]); const [audioDevices, setAudioDevices] = useState<Device[]>([]); const [boxes, setBoxes] = useState<Box[]>([]); const [metrics, setMetrics] = useState<any>(null); const [wave, setWave] = useState<number[]>([]); const [spectrum, setSpectrum] = useState<number[]>([]); const [audio, setAudio] = useState<any>(null); const [logs, setLogs] = useState<string[]>([]); const [wsState, setWsState] = useState('未连接'); const [busyAction, setBusyAction] = useState<BusyAction>(null); const [errorMessage, setErrorMessage] = useState<string | null>(null);
  ...
  useEffect(() => { refresh().catch(error => setErrorMessage(error instanceof Error ? error.message : String(error))); const timer = setInterval(()=>api.status().then(setStatus).catch(()=>{}), 2000); const close = connectEvents((m: EventMessage) => { if (m.type === 'detection.boxes') setBoxes(m.boxes); if (m.type === 'video.metrics') setMetrics(m); if (m.type === 'audio.waveform') setWave(m.values); if (m.type === 'audio.spectrum') setSpectrum(m.magnitudes); if (m.type === 'audio.metrics') setAudio(m); if (m.type === 'log') setLogs(l => [...l.slice(-199), `${new Date(m.timestamp).toLocaleTimeString()} [${m.level}] ${m.message}`]); }, setWsState); return () => { clearInterval(timer); close(); }; }, []);
  const start = async () => { if (!settings) return; setBusyAction('start'); setErrorMessage(null); try { const nextStatus = await api.applyStart(settings); setSaved(settings); setStatus(nextStatus); setLogs(l=>[...l.slice(-199),'已应用当前设置并启动管线。']); } catch (error) { setErrorMessage(error instanceof Error ? error.message : String(error)); } finally { setBusyAction(null); } };
  ...
  return <main><header><h1>局域网 USB 摄像头图像/音频算法实验台</h1><span>WebSocket：{wsState}</span></header><div className="dashboard"><VideoPanel url={settings?.mediamtx_webrtc_url ?? ''} boxes={boxes} width={metrics?.width ?? settings?.video_width ?? 1280} height={metrics?.height ?? settings?.video_height ?? 720}/><ControlPanel settings={settings} setSettings={setSettings} videoDevices={videoDevices} audioDevices={audioDevices} onRefresh={refreshDevices} onStart={start} onStop={stop} dirty={dirty} busyAction={busyAction} errorMessage={errorMessage}/><section className="card"><h2>音频波形</h2><WaveformCanvas values={wave}/></section><MetricsPanel status={status} metrics={metrics} audio={audio}/><section className="card"><h2>音频频谱</h2><SpectrumCanvas magnitudes={spectrum}/></section><LogPanel logs={logs}/></div></main>;
}
