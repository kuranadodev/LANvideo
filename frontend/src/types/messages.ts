...
export type WebRtcStats = { rttMs?: number; jitterMs?: number; framesDecoded?: number; framesDropped?: number; candidatePair?: string; bytesReceived?: number };
export type EventMessage =
  | { type: 'video.metrics'; timestamp: number; fps: number; analysis_fps?: number; frame_index: number; algorithm_ms: number; width: number; height: number; analysis_width?: number; analysis_height?: number }
